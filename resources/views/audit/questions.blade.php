                            ...
                            @foreach ($questions as $item)
                                ...
                                <div class="col-md-12">
                                    <div class="mb-3">
                                        ...
                                        <label for="question{{ $item->id }}" class="form-label">Pregunta N.
                                            {{ $item->order }}.{{ $item->question }}</label>
                                        <select name="question{{ $item->id }}"
                                            class="form-control answer-select @error('question{{ $item->id }}') is-invalid @enderror"
                                            id="question{{ $item->id }}" data-question-id="{{ $item->id }}">
                                            <option value="">Seleccione una respuesta</option>
                                            @foreach ($options as $option)
                                                <option value="{{ $option->id }}"
                                                    {{ old('client_id') == $option->name ? 'selected' : '' }}
                                                    @if ($existingAnswer && $existingAnswer->qualification_id == $option->id) selected @endif>
                                        ...
                                        <div class="mt-2">
                                            <label for="observacion{{ $item->id }}" class="form-label">Observaciones
                                                (opcional)
                                            </label>
                                            <textarea class="form-control observacion-field" id="observacion{{ $item->id }}"
                                                name="observacion{{ $item->id }}" rows="2" data-question-id="{{ $item->id }}"
                                                placeholder="Agregue notas adicionales si es necesario">{{ old('observacion' . $item->id) }}@if ($existingAnswer)
{{ $existingAnswer->observation }}
@endif
</textarea>
                                            <div class="feedback-message" id="feedback-observacion-{{ $item->id }}">
                                            </div>
                                        </div>
